...
<!-- inactive-ok-file: LIT-019 — Deferred: this is the seeded skim of the paper, filed with it on 2026-09-25 -->
...
- Defines the "TRAP" phenomenology (the phenomenology traditionally regarded as problematic) in the Mach-Zehnder setup and lists the interpretational claims it is said to support (§2.1-2.2).
- The move from a first-quantized (particle-path) to a second-quantized (mode) description is argued to be essential to the classical account (§2.3, §3.3, App. A.2).
- Toy model: two modes L, R, binary occupation numbers with N_L ⊕ N_R = 1, binary phases in Z_2, local deterministic dynamics, plus an epistemic restriction on joint knowledge (§3.1).
- Extends the account to the Elitzur-Vaidman bomb tester, Wheeler's delayed choice and the quantum eraser (§4).
- Proposes a methodology: define classicality, formalize it, then prove a no-go theorem; their preferred criterion (set-valued ontology with functional dynamics, Bayesian/Boolean inference, Leibnizianity) implies generalized noncontextuality, and the toy theory satisfies it (§5.1.2-5.1.3).
- Concludes that genuinely nonclassical phenomenology is what violates generalized noncontextuality, not interference as such (§5.1.3).
