...
formerly:
- LIT-tmp4goik
status_note: 'read in full 2026-09-25 (NOTE-077); worth reading as the clearest statement of the "open systems view" and of the distinction between a view, a framework, a theory and a model (§2, §4.1, §5.2). Read it knowing that its fundamentality verdict is carried by interpretive premises (OntFund-2, EpFund-2) and a deflationary step, not by any formal asymmetry between the frameworks.'
...
- version: 3
  ...
  note: >-
    ...
    Nothing was skipped.); the Skimmed NOTE became a Read one. Status set
...
summary: >-
  Cuffaro et al. (2021),
  [ARXIV-2112.11095](https://arxiv.org/abs/2112.11095). The paper argues
  philosophically, not formally, that the quantum theory of open systems
  "GT" is more fundamental than standard closed-system quantum theory
  "ST". GT treats states as density operators that evolve, in general,
  non-unitarily, and it admits not-completely-positive maps. The ontic and
  epistemic cases each win only on a second, interpretation- or
  practice-laden criterion: OntFund-2 and EpFund-2. On the first, formal
  criteria (OntFund-1, EpFund-1) the two frameworks tie, and the authors
  say so because Stinespring dilation and dropping the Lindblad terms
  translate in both directions (§4.1, §4.2.1). Explanatory fundamentality
  is granted only deflationarily (ExFund-1), and the authors recommend
  dropping the notion (§4.2.2). The step from there to "the open systems
  view is fundamental tout court" (§4.3) rests on the special sciences and
  on "there are no other options".
---
...
Filed from the survey of 2026-09-25 of work the anthology set aside as out of scope (tier B): 570 seconds of active reading over 5 sessions in the papers-feed tracker. It was filed `Deferred`, unread. [NOTE-077](../notes.d/NOTE-077.md) is the close reading of 2026-09-25, and it placed the work: **Active** — worth reading as the clearest statement of the "open systems view" and of the distinction between a view, a framework, a theory and a model (§2, §4.1, §5.2). Read it knowing that its fundamentality verdict is carried by interpretive premises (OntFund-2, EpFund-2) and a deflationary step, not by any formal asymmetry between the frameworks.
